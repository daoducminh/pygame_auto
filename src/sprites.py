...
class CarSprite(p.sprite.Sprite):
    ...
    def get_all_distance(self, sector):
        if isinstance(sector, Vertex) or isinstance(sector, Edge):
            left, right, front = [], [], []
            for s in sector.segments:
            ...
            return (
                float(min(left_dist)) if left_dist else None,
                float(min(right_dist)) if right_dist else None,
                float(min(front_dist)) if front_dist else None
            )
        return None
